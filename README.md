...
- Если в заключении указывается средство измерений, сведения о приборе должны быть полными: наименование, номер, реквизиты свидетельства о поверке и срок действия поверки.
- Система координат указывается полностью, включая зону и код, например `МСК-16, зона 1 (код 16.1)`.
- Формулировка `в местной системе координат МСК-16` без зоны считается ошибкой. Перед сохранением TXT ИИ должен проверить все упоминания `МСК-*`.
- Для технического плана обязательно добавляется фраза `СКП вычисляется по формуле Mp = Ms*√(a²+b²) и составила ХХ на площадь объекта ННН кв.м.`, где `ХХ` спрашивается у пользователя, а `ННН` берется из XML.
- Для технического плана ИИ обязательно спрашивает, порядок оформления упрощенный или уведомительный, и добавляет соответствующую фразу.
